...
func setHeaderMetaForHTML(_ htmlStr: String) -> String {
...
public func showLoaderHUD(strMessage:String)
{
    ...
    SVProgressHUD.setForegroundColor(#colorLiteral(red: 0.168627451, green: 0.5960784314, blue: 0.7294117647, alpha: 1))
...
func moveToUserType() {
    ...
    kAppDelegate.setTutorial()
}


//MARK:- Logout
func logout() {
    if (ModelUserData.getUserDataFromUserDefaults().token ?? "").count > 0 {
        logOutAPI()
        moveToUserType()
    }
...
func logOutAPI() {
    ...
    HttpRequestManager.sharedInstance.requestWithPostJsonParam(service: kLogout_URL, parameters: data as NSDictionary, showLoader: true, isLoginMandatory: true) { (response, error) in
        if error != nil
        {
            ...
            showMessage(response?.value(forKey: kMessage) as? String ?? "", themeStyle: .success)
            moveToUserType()
        }
    }
}
