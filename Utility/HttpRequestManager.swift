            ...
            alamoFireManager.upload(multipartFormData: { multipartFormData in
                ...
                for (key, value) in parameters
                {
                    if value is URL {
                        ...
                        let imgData = fixOrientationOfImage(image: (value as! UIImage))?.jpegData(compressionQuality: 5.5)
                ...
                if images.count > 0 {
                    ...
                    for image in images {
                        ...
                        {
                            if value is UIImage {
                                let imgData = fixOrientationOfImage(image: (value as! UIImage))?.jpegData(compressionQuality: 0.7)
        ...
        if isConnectedToNetwork()
        {
            ...
            alamoFireManager.request(strURL, method: .get, parameters:nil , encoding: JSONEncoding.default, headers: additionalHeader)
                .responseJSON { response in
                    ...
                    {
                    case .success(let value):
                        responseData(nil, dictionaryOfFilteredBy(dict: (value as? NSDictionary)!))
                        break
                    case .failure(let error):
